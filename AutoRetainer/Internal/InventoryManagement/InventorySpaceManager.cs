...
namespace AutoRetainer.Internal.InventoryManagement
{
    public unsafe static class InventorySpaceManager
    {
        ...
        static nint AgentRetainerItemCommandModule => (nint)AgentModule.Instance()->GetAgentByInternalId(AgentId.Retainer) + 40;
        static bool IsAgentRetainerActive => AgentModule.Instance()->GetAgentByInternalId(AgentId.Retainer)->IsAgentActive();

        public static readonly List<SellSlotTask> SellSlotTasks = [];

        static readonly InventoryType[] NormalInventoryTypes = [InventoryType.Inventory1, InventoryType.Inventory2, InventoryType.Inventory3, InventoryType.Inventory4];
